...
## GET THE CLEANED DATA
    ...
    ##This program pulls the cleaned data from github directly


    file.path <- "https://raw.githubusercontent.com/tactical-Data/Exascalar/master"
    ...
    BigEx<-readData(path.name=file.path, dir.extension = "/results", file.name="/BigExascalar.csv")
    Jun15<-readData(path.name=file.path, dir.extension = "/results", file.name="/Jun15_merged.csv")
    Nov14<-readData(path.name=file.path, dir.extension = "/results", file.name="/Nov14_merged.csv")
    Nov12<-readData(path.name=file.path, dir.extension = "/results", file.name="/Nov12_merged.csv")
    Jun14<-readData(path.name=file.path, dir.extension = "/results", file.name="/Jun14_merged.csv")
    Jun13<-readData(path.name=file.path, dir.extension = "/results", file.name="/Jun13_merged.csv")
...
## EXASCALAR PLOT
    ...
    p <- p + theme(
        ...
        panel.grid.major = element_line(colour = "bisque2"),
        panel.grid.minor = element_line(colour = "bisque2")
        )
...
## EXASCALAR PLOT
    ...
    p <- p + scale_colour_manual(name='List',values=c("2015-06-01"="darkred","2013-06-01"="blue3"))
...
```{r, echo=FALSE, fig.align='center', fig.height=6, fig.width=7.1, warning=FALSE}
...
## EXASCALAR PLOT
    ...
    ## Add an age
    BigData$age_years<-2016-as.integer(BigData$year.t)
    
    age2015<-as.vector(BigData$age_years)
    
    
    ## CREATE EXASCALAR PLOT
    ## take the log of the data points explicitly rather than use a log-scale command
    p <- ggplot(BigData, aes(x=log10(mflopswatt), y=log10(rmax*10^3), color=factor(age_years)))
    
    
    
    ## Define plot points and symbols
    ## alpha = 0.8 helps visually recognize where points overlap
    p <- p + geom_point(alpha=0.9, size=5, pch=20)
    p <- p + scale_color_manual(values = rainbow(10))
    ...
    p <- p + scale_x_discrete(breaks = 1:5, labels=c("1e+01","1e+02","1e+03","1e+04","1e+05")) 
    p <- p + scale_y_discrete(breaks = 8:12, labels=c("1e+08","1e+09","1e+10","1e+11","1e+12")) 
    p <- p + ggtitle("Jun 2015 Exascalar (Age in Years)")

    ## annotate source

    p <- p + annotate("text", x = 3.9, y= 7.6, label = "sources:~Top500.org~and~Green500.org", parse=TRUE, angle=0, size=3)

    ## add Epsilon values for Exascalar
    p <- p + annotate("text", x=4.64, y= 12.2, label = "epsilon == 0", parse=TRUE, angle= -45, size=4)
    p <- p + annotate("text", x=3.44, y= 12.1, label = "epsilon == -1", parse=TRUE, angle= -45, size=4)
    p <- p + annotate("text", x=2.08, y= 12, label = "epsilon == -2", parse=TRUE, angle= -45, size=4)
    p <- p + annotate("text", x=1.30, y= 11.40, label = "epsilon == -3", parse=TRUE, angle= -45, size=4)
    p <- p + annotate("text", x=1.18, y= 10.11, label = "epsilon == -4", parse=TRUE, angle= -45, size=4)

    ## add Power annotation
    p <- p + annotate("text", x=4.18, y= 11.70, label = "20 MWatt", parse=F, angle= 45, size=4)
    p <- p + annotate("text", x=4.58, y= 11.10, label = "2.0 MWatt", parse=F, angle= 45, size=4)
    p <- p + annotate("text", x=4.58, y= 10.10, label = "0.2 MWatt", parse=F, angle= 45, size=4)
    p <- p + annotate("text", x=4.58, y= 9.10, label = "20 kWatt", parse=F, angle= 45, size=4)
...
## EXASCALAR PLOT
    ...
    ## Add an age
    BigData$age_years<-2014-as.integer(BigData$year.t)
    
    age2013<-as.vector(BigData$age_years)
    age2013<-age2013[!is.na(age2013)]
    ...
    p <- p + scale_x_discrete(breaks = 1:5, labels=c("1e+01","1e+02","1e+03","1e+04","1e+05")) 
    p <- p + scale_y_discrete(breaks = 8:12, labels=c("1e+08","1e+09","1e+10","1e+11","1e+12")) 
    p <- p + ggtitle("Jun 2013 Exascalar (Age in Years)")
...
As a general observation, the 2015 population, with a median age of `r median(age2015)` is older than the 2013 population, with a median age of `r median(age2013)`, with a difference in means of `r mean(age2015)-mean(age2013)` years. 
...
```{r, 'date trend eff', echo=FALSE, fig.align="center", fig.height=4, fig.width=6}
...
```{r, 'date trend exa', echo=FALSE, fig.align="center", warning=FALSE, fig.height=4, fig.width=6}
...
## EXASCALAR PLOT
    ...
    ##elimated the 60 core from Jun15
    Jun15$coressocket.t[Jun15$coressocket.t==60]<-6

    ## Make sure both dataframes have the same columns
    common_columns <- intersect(names(Jun15), names(Jun13))

    BigData<-rbind(Jun13[, common_columns],Jun15[, common_columns])
    ...
    p <- p + scale_x_discrete(breaks = 1:5, labels=c("1e+01","1e+02","1e+03","1e+04","1e+05")) 
    p <- p + scale_y_discrete(breaks = 8:12, labels=c("1e+08","1e+09","1e+10","1e+11","1e+12")) 
    p <- p + ggtitle("Jun 2015 and Nov 2012 Exascalar (Cores Per Socket)")
...
```{r, echo=FALSE, fig.align='center', fig.height=4.25, fig.width=7, warning=FALSE}
...
## EXASCALAR PLOT
    ...
    ## Make sure both dataframes have the same columns
    common_columns <- intersect(intersect(names(Jun15), names(Jun13)), names(Jun14))
    ##combine columns
    BigData<-rbind(Jun13[, common_columns],Jun14[, common_columns],Jun15[, common_columns])
    
    ## CREATE HISTOGRAM
    
    p <- ggplot(BigData, aes(coressocket.t, fill=Date))
    ...
    ## Define plot points and symbols
    ## alpha = 0.8 helps visually recognize where points overlap
    p <- p + geom_histogram(breaks=seq(0, 40, by =4), 
                 col="blue2", 
                 position="dodge")
    ...
    p <- p + theme(
        ...
        axis.title.x = element_text(color="black", vjust=-0.45),
        axis.title.y = element_text(color="black" , vjust=0.45),
        panel.grid.major = element_line(colour = "antiquewhite"),
        panel.grid.minor = element_line(colour = "antiquewhite"),
        axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position="bottom"
        )

    p<-p+labs(x="Cores", y="Count")

    p <- p + ggtitle("Yearly Shift of Core Count Distribution")
    
    p <- p + scale_fill_manual(values=c("lightblue3", "lightseagreen", "indianred3"))

    

    ## annotate source

    p <- p + annotate("text", x = 34, y= 20, label = "sources:~Top500.org~and~Green500.org", parse=TRUE, angle=0, size=2.6)
...
```{r, echo=FALSE, fig.align='center', results="asis"}
    ...
    ## create table
    ## note need to control for NA's in some cases
    
    core_stats_table<-matrix(c("Jun2013", 
                               median(BigData$coressocket.t[BigData$Date=="Jun 2013"&!is.na(BigData$coressocket.t)]),
                               mean(BigData$coressocket.t[BigData$Date=="Jun 2013"&!is.na(BigData$coressocket.t)]),
                               "Jun2014", 
                               median(BigData$coressocket.t[BigData$Date=="Jun 2014"]), 
                               mean(BigData$coressocket.t[BigData$Date=="Jun 2014"]),
                               "Jun2015", 
                               median(BigData$coressocket.t[BigData$Date=="Jun 2015"]), 
                               mean(BigData$coressocket.t[BigData$Date=="Jun 2015"])
                               ), nrow=3, ncol=3, byrow=TRUE
                             )
    
    core_stats_table<-as.data.frame(core_stats_table)
    colnames(core_stats_table)<-c("Date", "Median Cores", "Mean Cores")
    
    temptable<-xtable(core_stats_table)
    print(temptable, type="html")  
...
## EXASCALAR PLOT
    ...
    ## draw the power lines
    for (i in -1:2){
    p <- p + geom_abline(intercept=isopowerline_ab(2*10^-i), colour = "grey65", size = 0.5, alpha=0.5, linetype=2)
    }
    #print(p)
    
    ##draw the exascalar lines
    for (i in 0:7) {
        p <- p + geom_abline(intercept = isoexaline_ab(-i), slope=-1, colour = "grey65", size = 0.5, alpha=0.5, linetype=2)
    }
...
## EXASCALAR PLOT
    ...
    p <- p + scale_x_discrete(breaks = 1:5, labels=c("1e+01","1e+02","1e+03","1e+04","1e+05")) 
    p <- p + scale_y_discrete(breaks = 8:12, labels=c("1e+08","1e+09","1e+10","1e+11","1e+12")) 
    p <- p + ggtitle("June 2015 and June 2013 Exascalar (Total Cores)")
